...
from models.unsup_frag_copy import UnsupFragAlign, UnsupFragAlign_FineTune, FragAlignLoss, GlobalRankLoss, BatchSoftmax, BatchSoftmaxSplit
...
parser.add_argument("--celeb_model_name", type=str, default="waldo-unsup_frag_two5-proj_dim:128_biasTrue_data:train_loss:batch-0.25-agree-normal-full_bsz:20_epoch3_op:adam_lr0.0003_nonameTrue_True_textModelbert-uncased_finetune-False_mean-True-True-layerS-4.pt")
parser.add_argument("--celeb_dir", type=str, default="/export/home1/NoCsBack/working/tingyu/face_naming/celeb")
...
parser.add_argument("--test_model", default=False, type=lambda x: (str(x).lower() == 'true'))
...
def eval_epoch(model, loss_type, frag_loss, global_loss, val_dataloader):
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(val_dataloader, desc="Iteration")):
            face_emb, ner_features, ner_context_features = batch["face_emb"], batch["ner_features"], batch["ner_context_features"]
...
def train_all(model, loss_type, frag_loss, global_loss, all_dataloader, optimizer, scheduler):
    train_losses = []
...
if __name__ == "__main__":
    ...
    out_file_name = args.celeb_model_name[:-3]+"tune_epoch{}".format(args.num_epoch)
    ...
    unsup_frag_net = torch.load(os.path.join(args.celeb_dir, args.celeb_model_name))
    ...
    dict_name = os.path.join(args.dict_dir, args.dict_name)
    ...
    face_data_full = CelebDataset(base_dir,
                            ...
                            add_noname=args.add_noname,
                            cons_noname=args.cons_noname,
                            dict_name=os.path.join(args.dict_dir, "celeb_dict.json"))
    ...
    logger.info(f"train size:{len(face_data)}")
    logger.info(f"test size:{len(face_data_full)}")
    ...
    if args.data_type == "test":
        ...
        if args.test_model:
            model = torch.load(os.path.join(out_dir, args.celeb_model_name[:-3]+"tune_epoch{}.pt".format(args.num_epoch)))
            model = model.cuda()
        ...
        torch.save(model, os.path.join(out_dir, args.celeb_model_name[:-3]+"tune_epoch{}.pt".format(args.num_epoch)))
    ...
    with torch.no_grad():
        ...
        out_dir_name = os.path.join(out_dir,  os.path.join(out_dir, args.celeb_model_name[:-3]+"tune_epoch{}.json".format(args.num_epoch)))
