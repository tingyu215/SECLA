...
class UnsupFragAlign(nn.Module):
    def __init__(self, ner_dim, proj_type="one", add_bias=False, n_features=512, proj_dim=512):
    ...
    def forward(self, enc_face_emb, input_ids):
        enc_ner_list = []
        for i in range(input_ids.size()[0]):
            name_ids = input_ids[i]
            if input_ids is not None:
                input_shape = name_ids.size()
            else:
            ...
            inputs_embeds = self.word_embeddings(name_ids)
            token_type_embeddings = self.token_type_embeddings(token_type_ids)

            enc_ner_emb = inputs_embeds + token_type_embeddings
            position_embeddings = self.position_embeddings(position_ids)
            enc_ner_emb += position_embeddings
            enc_ner_emb = self.LayerNorm(enc_ner_emb)
            enc_ner_emb = self.dropout(enc_ner_emb)
            enc_ner_emb = torch.mean(enc_ner_emb, dim=1)
            enc_ner_list.append(enc_ner_emb)
        enc_ner_emb = torch.stack(enc_ner_list)
